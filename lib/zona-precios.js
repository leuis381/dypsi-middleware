...
import { logger, ValidationError, AppError, MetricsCollector, sanitizeInput } from './utils.js';
...
export function findZoneByDistrict(district) {
  metrics.record('zone.find.call', 1);
  
  if (!district || typeof district !== 'string') {
    logger.warn('ZONE_INVALID_DISTRICT', { district, type: typeof district });
    metrics.record('zone.find.invalid_input', 1);
    return ZONE_RULES.find(z => z.zone === 'D');
  }

  const normalizedDistrict = sanitizeInput(district, 100).toLowerCase();
  ...
  for (const zone of ZONE_RULES) {
    if (zone.districts.includes(normalizedDistrict)) {
      logger.debug('ZONE_FOUND', { district: normalizedDistrict, zone: zone.zone, delivery: zone.delivery });
      metrics.record('zone.find.success', 1);
      metrics.record(`zone.zone_${zone.zone}`, 1);
      return zone;
    }
  }

  logger.debug('ZONE_NOT_FOUND_USING_DEFAULT', { district: normalizedDistrict });
  metrics.record('zone.find.default_used', 1);
  return ZONE_RULES.find(z => z.zone === 'D');
}
...
export function calculateRoutePrice(storeCoords = {}, destCoords = {}) {
  ...
  metrics.record('zone.route_price.call', 1);
  
  try {
    logger.debug('ROUTE_PRICE_CALCULATION_START', { storeCoords, destCoords });

    if (!storeCoords || !destCoords) {
      throw new ValidationError('Las coordenadas de tienda y destino son requeridas');
    }
    ...
    logger.info('ROUTE_PRICE_CALCULATED', { distanceKm, durationMin, price, duration_ms: duration });
    metrics.record('zone.route_price.success', 1);
    metrics.record('zone.route_price.duration_ms', duration);

    return { distanceKm, durationMin, price };

  } catch (error) {
    metrics.record('zone.route_price.error', 1);
    if (error instanceof ValidationError) throw error;
    logger.error('ROUTE_PRICE_CALCULATION_FAILED', { error: error.message });
    throw new AppError('Error calculando precio de ruta', 500, 'ROUTE_CALCULATION_FAILED');
  }
}
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (!item || typeof item !== 'object') {
        throw new ValidationError(`Item ${i}: debe ser un objeto válido`);
      }

      const unitPrice = Number(item.price ?? item.priceHint ?? 0);
      if (isNaN(unitPrice) || unitPrice < 0) {
        throw new ValidationError(`Item ${i}: precio inválido (${item.price})`);
      }

      const quantity = Number(item.quantity ?? 1);
      if (isNaN(quantity) || quantity < 1) {
        throw new ValidationError(`Item ${i}: cantidad debe ser >= 1`);
      }

      const extrasPrice = Number(item.extrasPrice ?? 0);
      if (isNaN(extrasPrice) || extrasPrice < 0) {
        throw new ValidationError(`Item ${i}: extras inválidos`);
      }
    ...
    metrics.record('zone.order_total.success', 1);
    metrics.record('zone.order_total.duration_ms', duration);
    metrics.record('zone.order_total.items', items.length);
    ...
    metrics.record('zone.order_total.error', 1);
    if (error instanceof ValidationError) throw error;
    logger.error('ORDER_TOTAL_CALCULATION_FAILED', { error: error.message });
    throw new AppError('Error calculando total de orden', 500, 'ORDER_TOTAL_FAILED');
  }
}

/**
 * Buscar producto en el menú por id o sku
 * @param {Object} menu - Menú completo
 * @param {string} productId - ID o SKU
 * @returns {Object|null}
 */
export function findProductInMenu(menu = {}, productId = '') {
  if (!menu || !productId) return null;
  ...
  for (const category of categories) {
    ...
    for (const product of products) {
      ...
      if (id === normalized || sku === normalized) {
        return product;
      }
    }
  }
...
/**
 * Aplicar precio de variante si corresponde
 * @param {Object} product - Producto
 * @param {string} variantId - ID o nombre de variante
 * @returns {number|null}
 */
export function applyVariantPrice(product = {}, variantId = null) {
  if (!product || typeof product !== 'object') return null;
  const basePrice = typeof product.precio === 'number' ? product.precio : null;
  ...
  const match = variants.find(v =>
    String(v?.id || '').toLowerCase() === normalized ||
    String(v?.nombre || '').toLowerCase() === normalized
  );

  if (match && typeof match.precio === 'number') return match.precio;
  return basePrice;
}
