...
function validateDb() {
  if (!db) {
    logger.warn('FIRESTORE_NOT_INITIALIZED_USING_MEMORY', {});
...
function getOrCreateMemorySession(phoneStr) {
  if (!memorySessions.has(phoneStr)) {
    const initial = {
      estado: 'inicio',
      pedido: null,
      address: null,
    ...
    memorySessions.set(phoneStr, initial);
    return initial;
  }
  return memorySessions.get(phoneStr);
}

function saveMemorySession(phoneStr, data) {
  const current = memorySessions.get(phoneStr) || {};
  const merged = {
    ...current,
    ...data,
    updatedAt: new Date().toISOString()
  ...
  memorySessions.set(phoneStr, merged);
  return merged;
}
...
export async function getSession(phone) {
  ...
  try {
    const hasDb = validateDb();
    const phoneStr = validatePhone(phone);

    logger.debug('SESSION_GET_START', { phone: phoneStr });

    if (!hasDb) {
      const session = getOrCreateMemorySession(phoneStr);
      const duration = Date.now() - startTime;
      logger.info('SESSION_MEMORY_RETRIEVED', { phone: phoneStr, estado: session.estado, duration_ms: duration });
      metrics.record('session.get.memory', 1);
...
export async function saveSession(phone, data) {
  ...
  try {
    ...
    if (!hasDb) {
      saveMemorySession(phoneStr, data);
      const duration = Date.now() - startTime;
      logger.info('SESSION_MEMORY_SAVED', { phone: phoneStr, duration_ms: duration });
      metrics.record('session.save.memory', 1);
...
export async function saveAddressForPhone(phone, address, components = {}) {
  ...
  try {
    ...
    if (!hasDb) {
      saveMemorySession(phoneStr, payload);
      const duration = Date.now() - startTime;
      logger.info('ADDRESS_MEMORY_SAVED', { phone: phoneStr, duration_ms: duration });
      metrics.record('session.save_address.memory', 1);
...
export async function saveOrderDraft(phone, parsedOrder) {
  ...
  try {
    ...
    if (!hasDb) {
      saveMemorySession(phoneStr, payload);
      const duration = Date.now() - startTime;
      logger.info('ORDER_DRAFT_MEMORY_SAVED', { phone: phoneStr, duration_ms: duration });
      metrics.record('session.save_order.memory', 1);
...
export async function deleteSession(phone) {
  ...
  try {
    ...
    if (!hasDb) {
      memorySessions.delete(phoneStr);
      const duration = Date.now() - startTime;
      logger.info('SESSION_MEMORY_DELETED', { phone: phoneStr, duration_ms: duration });
      metrics.record('session.delete.memory', 1);
