...
export const CONFIG = {
  ...
  FIREBASE_PRIVATE_KEY: (() => {
    ...
    if (!key) return null;
    // Maneja TODOS los formatos de saltos de línea posibles:
    // - \\n (escapado doble)
    // - \n (escapado simple)
    // - Saltos de línea reales
    // - \r\n (Windows)
    let processed = key;
    // Primero convertir saltos de línea reales a placeholder
    processed = processed.replace(/\r\n/g, '___NEWLINE___');
    processed = processed.replace(/\n/g, '___NEWLINE___');
    // Luego procesar los escapados
    processed = processed.replace(/\\\\n/g, '___NEWLINE___');
    processed = processed.replace(/\\n/g, '___NEWLINE___');
    // Finalmente convertir placeholders a saltos reales
    processed = processed.replace(/___NEWLINE___/g, '\n');
    return processed;
  })(),
  
  // OCR API (REQUERIDO para procesar imágenes)
  OCR_API_KEY: (() => {
    const key = process.env.OCR_API_KEY;
    if (!key) {
      console.warn('⚠️  OCR_API_KEY no configurada - OCR deshabilitado');
      return null;
    }
    return key.trim();
  })(),
  ...
  // BOT STATUS - Botón de encendido/apagado
  BOT_ENABLED: process.env.BOT_ENABLED === 'true' || process.env.BOT_ENABLED === '1' || true,
  BOT_MAINTENANCE_MODE: process.env.BOT_MAINTENANCE_MODE === 'true' || process.env.BOT_MAINTENANCE_MODE === '1' || false,
  BOT_VERSION: 'v4.0 ULTRA+',
