...
import { logger, AppError, asyncHandler, sendSuccess } from './utils.js';
...
async function checkWhatsApp() {
  try {
    ...
    if (response.status === 200) {
      logger.info('HEALTH_WHATSAPP_OK', { status: response.status });
      return { ok: true, status: response.status, details: 'WhatsApp Graph API accesible' };
    }

    logger.warn('HEALTH_WHATSAPP_ERROR', { status: response.status });
    return {
      ok: false,
    ...
    logger.warn('HEALTH_WHATSAPP_PROBE_FAILED', { error: error.message, code: error.code });
    return {
      ok: false,
...
async function checkLocalMenu() {
  try {
    ...
    logger.info('HEALTH_MENU_OK', { path: LOCAL_MENU_PATH, size_bytes: stat.size });
    return { ok: true, path: LOCAL_MENU_PATH, size_bytes: stat.size, valid_json: true };

  } catch (error) {
    logger.warn('HEALTH_MENU_CHECK_FAILED', { error: error.message, code: error.code });
    return {
      ok: false,
      reason: 'file_error',
...
async function checkDatabaseDns() {
  try {
    ...
    logger.info('HEALTH_DATABASE_DNS_OK', { host, address: records.address });
    return { ok: true, host, port, address: records.address, family: `IPv${records.family}` };

  } catch (error) {
    logger.warn('HEALTH_DATABASE_DNS_FAILED', { error: error.message, code: error.code });
    return {
      ok: false,
...
export default asyncHandler(async (req, res) => {
  logger.info('HEALTH_CHECK_REQUEST', { method: req.method, ip: req.ip });

  // Solo permitir GET
  if (req.method !== 'GET') {
    logger.warn('HEALTH_INVALID_METHOD', { method: req.method });
  ...
  try {
    ...
    logger.debug('HEALTH_ENV_CHECK', env);

    // Ejecutar checks en paralelo con timeout
    logger.debug('HEALTH_RUNNING_PROBES');
