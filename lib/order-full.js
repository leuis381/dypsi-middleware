...
const LOCAL_MENU_PATH = CONFIG.MENU_DATA_PATH || path.resolve(process.cwd(), "menu.json");
...
async function fetchWhatsAppCatalog() {
  ...
  if (!process.env.WHATSAPP_BUSINESS_ID || !process.env.WHATSAPP_TOKEN) {
    logger.warn('WhatsApp credentials not configured - Catalog feature disabled');
    throw new AppError("WhatsApp credentials not configured. This feature is optional.", 501, 'FEATURE_DISABLED');
  }
